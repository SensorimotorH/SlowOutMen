...
class NavigationtoCabRound1:
    ...
    def _step_8(self) -> None:
        ...
        box_pose_base = self.posetran.pos_cam_to_base(box_pose_cam)
        tmp_lft_arm_target_pose = box_pose_base + np.array([0.0, 0.11, 0.1]) # y,x,z
        tmp_rgt_arm_target_pose = box_pose_base + np.array([0.0,-0.11, 0.1]) # y,x,z
        tmp_lft_arm_state = self.getArmEndTarget(tmp_lft_arm_target_pose, "pick", "l", np.array(self.receiver.get_joint_states()['positions'][3:9]), R.from_euler("zyx", [np.pi/2, -0.0551 + np.pi, np.pi / 8]).as_matrix())
        ...
        tmp_lft_arm_target_pose = box_pose_base + np.array([0.0, 0.08, 0.1]) # y,x,z
        tmp_rgt_arm_target_pose = box_pose_base + np.array([0.0,-0.08, 0.1]) # y,x,z
        tmp_lft_arm_state = self.getArmEndTarget(tmp_lft_arm_target_pose, "pick", "l", np.array(self.receiver.get_joint_states()['positions'][3:9]), R.from_euler("zyx", [np.pi/2, -0.0551 + np.pi, np.pi / 8]).as_matrix())
        tmp_rgt_arm_state = self.getArmEndTarget(tmp_rgt_arm_target_pose, "pick", "r", np.array(
        self.receiver.get_joint_states()['positions'][10:16]), R.from_euler("zyx", [-np.pi/2, -0.0551 + np.pi, -np.pi / 8]).as_matrix())
        time.sleep(1)
    ...
    def _step_9_desk(self) -> None:
        ...
        for i in range(100):
            self.controller.set_head_slide_position((100-i) * 0.01 * current_slide_pose)
            time.sleep(0.1)
            
        if self.cabinet_index == "left":
            if self.table_index == "left":
                self.controller.set_turn_left_angle(180)
                self.move_to_point.move_to_point([self.LEFT_X,0.15,0])
            elif self.table_index == "right":
                self.controller.set_turn_left_angle(90)
                self.move_to_point.move_to_point([-0.1,self.RIGHT_Y,-90])
        ...
        print(f"self.current_position:{self.current_position['x']}, {self.current_position['y']}")
        ...
        tmp_rgt_arm_state=[-0.4641777170640647, 0, 1.0497040081105575, 0.8421601585342549, 0.670234867120661, -0.7160617625400274]
        tmp_lft_arm_state= [0.5395976853056692, 0, 1.066561862005807, -0.6202181291863684, -0.6228169552416241, 0.44089390986698723]
        ...
        if self.table_index == "left":
            self.move_to_point.move_to_point([self.current_position['x'], -0.2, 0])
        elif self.table_index == "right":
            self.move_to_point.move_to_point([-0.3, self.current_position['y'], -90.0])
        ...
        try:
            print("ACT pick disk")
            command = f"cd /workspace/DISCOVERSE/policies/act && python3 policy_evaluate_ros.py -tn mmk2_pick_prop -ts 20250404-004833"
    ...
    def navigation_to_obj(self,obj_name) -> None:
        ...
        if right_result == True and left_result == False:
            angle = -90
            time.sleep(1)
            self.move_to_point.move_to_point([obj_world_pose[0]+0.8, obj_world_pose[1], angle])
        if left_result == True and right_result == False:
            angle = 0
            self.move_to_point.move_to_point([obj_world_pose[0], obj_world_pose[1]+0.8, angle])
    ...
    def observation(self) -> None:
        if self.table_index == "left":
            ...
            self.controller.set_head_pitch_angle(0.25)
            time.sleep(1)
        vector = self.vision_module.get_empty_area_position(self.table_index)
        obj_pose_base = self.posetran.pos_cam_to_base(vector)
        obj_world_pose = self.posetran.pos_base_to_world(obj_pose_base)
        if self.table_index == "left":
            self.LEFT_X= obj_world_pose[0]
        elif self.table_index == "right":
            self.RIGHT_Y = obj_world_pose[1]
        self.controller.reset_head()
        
    def test(self) ->None:
        # self.prop_name="carton"
        # prop_pose = self.vision_module.get_Toc_prop_from_yolo(self.prop_name)
        # prop_base = self.posetran.pos_cam_to_base(prop_pose)
        # tmp_lft_arm_target_pose = prop_base + np.array([0.5, 0.5, 0.1])
        # tmp_lft_arm_state = self.getArmEndTarget(tmp_lft_arm_target_pose, "pick", "l", np.array(self.receiver.get_joint_states()['positions'][3:9]), np.eye(3))
        # self.controller.set_left_arm_pos(tmp_lft_arm_state)
        # time.sleep(10)
...
def main():
    ...
    try:
        ...
        # controller.set_arm_position(tmp_lft_arm_state, tmp_rgt_arm_state)
